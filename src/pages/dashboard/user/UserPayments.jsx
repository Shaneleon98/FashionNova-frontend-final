             ...
             <ul>
                {
                    orders && orders.map((item,index) => (
                        <li key={index}>
                             <h5 className='font-medium text-gray-800 mb-2'>#{index + 1}</h5>
                             <div>
                             ...
                             <div className='flex md:flex-row items-center space-x-2'>
                                ...
                                <p className='text-gray-600'>
                                    | Status: <span className={`ml-2 py-[2px] px-2 text-sm rounded
                                        ${item?.status === 'completed' ? 'bg-green-100 text-green-600' : item?.status === 'pending' ? 'bg-red-200 text-red-700' : item?.status ==='processing' ? 'bg-yellow-100 text-yellow-700': 'bg-blue-200 text-blue-700' }
                                        `}> {item?.status}</span>
                                </p>
